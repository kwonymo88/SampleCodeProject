...
#include "PlayerInteractionInterface.h"
...
ADoor::ADoor()
{
	...
	static FName NAME_CapsuleComponentName(TEXT("CollisionForInteraction"));
	if (InteractionComponent = CreateDefaultSubobject<UCapsuleComponent>(NAME_CapsuleComponentName))
	{
		InteractionComponent->SetupAttachment(GetRootComponent());
		SetRootComponent(InteractionComponent);
		SetCollisionComponent(InteractionComponent);
	}
}
...
bool ADoor::ShouldInteract() const
{
...
bool ADoor::ShouldActionDoorState() const
{
	if (DoorState == EDoorState::Open
